...
RasterizeGaussiansCUDA(
	const torch::Tensor& background, // raster_settings
	const torch::Tensor& means3D,
	const torch::Tensor& features,  //my
    const torch::Tensor& colors,
    const torch::Tensor& opacity,
	const torch::Tensor& scales,    // 
	const torch::Tensor& rotations, // 
	const float scale_modifier,  // raster_settings ok
	const torch::Tensor& cov3D_precomp,
	const torch::Tensor& viewmatrix, // raster_settings.camera
	const torch::Tensor& projmatrix, // raster_settings.camera
	const float tan_fovx,  // raster_settings. camera
	const float tan_fovy,  // raster_settings. camera
    const int image_height,// raster_settings.camera  90
    const int image_width, // raster_settings.camera  360
	const torch::Tensor& sh,
	const int degree,
	const torch::Tensor& campos, // raster_settings.camera
	...
	const bool debug)
{
  ...
  torch::Tensor out_invdepth = torch::full({0, H, W}, 0.0, float_opts);  // first
  torch::Tensor out_feature = torch::full({S, H, W}, 0.0, float_opts);   //my
  float* out_invdepthptr = nullptr;

  out_invdepth = torch::full({1, H, W}, 0.0, float_opts).contiguous();  // first
  out_invdepthptr = out_invdepth.data<float>();   // first
  ...
  if(P != 0)
  {
	  ...
	  rendered = CudaRasterizer::Rasterizer::forward(
		...
		colors.contiguous().data_ptr<float>(),  //RF
		features.contiguous().data_ptr<float>(), //my
  ...
  return std::make_tuple(rendered, out_color, out_feature, radii, geomBuffer, binningBuffer, imgBuffer, out_invdepth);
}
	...
	const bool debug)
{
  ...
  torch::Tensor dL_dfeatures = torch::zeros({P, S}, features.options());  //my 
  ...
  {  
	...
	CudaRasterizer::Rasterizer::backward(P, S, degree, M, R,
	    ...
	    dL_dfeatures.contiguous().data_ptr<float>(), //my
