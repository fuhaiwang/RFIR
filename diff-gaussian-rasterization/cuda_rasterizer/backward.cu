...
renderCUDA(
	...
	float3* __restrict__ dL_dmean2D,  // error 
    ...
    float* __restrict__ dL_dfeature)
{
	...
	__shared__ float collected_features[3 * BLOCK_SIZE];  // 3D Gaus point color, forward one channel is only used once so there is no need to take in advance, but here the color of each channel c has more than one use, so take in advance
	...
	float accum_rec[C] = { 0 }; // 
	float accum_rec_d = 0.f, accum_rec_o = 0.f, accum_rec_f[3] = { 0 }; //my
	float dL_dpixel[C];
	float dL_dpixel_d, dL_dpixel_o, dL_dpixel_f[3]; //my
