...
renderCUDA(
	...
	const float* __restrict__ features_pbr,  // RF 
...
{   
	...
	float C[CHANNELS] = { 0 }, F[33] = { 0 }; // Auxiliary record, cumulative color of current pixel
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			for (int ch = 0; ch < CHANNELS; ch++)
				C[ch] += features_pbr[collected_id[j] * CHANNELS + ch] * alpha * T;
			
			for (int ch = 0; ch < S; ch++)  // my
