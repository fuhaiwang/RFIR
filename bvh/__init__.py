...
class RayTracer:
    ...
    def trace_visibility(self, rays_o, rays_d, means3D, symm_inv, opacity, normals):
        rays_o = rays_o + rays_d * 0.05    # 公式(13)？
        cotrib, opa = _C.trace_bvh_opacity(self.tree, self.aabb,   # 确定每条射线，调用 ray tracing 接口模拟是否可见  C++
                                                 rays_o, rays_d,
                                                 means3D, symm_inv,
                                                 opacity, normals)
        return {
    ...
    @torch.no_grad()
    def trace_visibility_segment(self, rays_o, rays_d, means3D, symm_inv, opacity, normals, distance):

        cotrib, opa = _C.trace_bvh_opacity_segment(self.tree, self.aabb,
                                                 rays_o, rays_d,
                                                 means3D, symm_inv,
                                                 opacity, normals, distance)
        return {
            "visibility": opa.unsqueeze(-1),
            "contribute": cotrib.unsqueeze(-1),
        }
