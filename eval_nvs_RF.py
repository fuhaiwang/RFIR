...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background, pbr_kwargs=None):
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        ...
        bb1 = results["reflection_phase"]  # 这个是渲染后的数值。没有实际意义的。
        ...
        Render_abs_db = 10 * torch.log10(Render_abs  + 1e-12)
    ...
    mse_mean = torch.mean(torch.abs(gt_abs_db - Render_abs_db))

    save_dir = os.path.join(model_path, name, f"ours_{iteration}")
    os.makedirs(save_dir, exist_ok=True)

    loss_path = os.path.join(save_dir, "mse_loss.txt")
    with open(loss_path, "w") as f:
        f.write(f"MSE_loss: {mse_mean.item():.6f}\n")
