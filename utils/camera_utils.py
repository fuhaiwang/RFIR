...
def cameraList_from_camInfos(cam_infos, resolution_scale, args): # 读取cam_infos
    camera_list = []
...
def loadAnt(args, id, ant_info, resolution_scale):

    return Antenna(colmap_id=ant_info.uid, R=ant_info.R, T=ant_info.T, FoVx=ant_info.FovX, FoVy=ant_info.FovY, RF=ant_info.RF, RF_name=ant_info.image_name,
                   image_width=ant_info.width, image_height=ant_info.height, uid=None, Tx_pos=ant_info.Tx_position, Rx_pos=ant_info.Rx_position, data_device=args.data_device)


def antennaList_from_antInfos(ant_infos, resolution_scale, args):  # 读取cam_infos
    antenna_list = []

    for id, ant in enumerate(tqdm(ant_infos, desc="resolution scale: {}".format(resolution_scale), leave=False)):
        antenna_list.append(loadAnt(args, id, ant, resolution_scale))
