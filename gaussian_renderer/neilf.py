...
def render_view(viewpoint_camera: Camera, pc: GaussianModel, pipe, bg_color: torch.Tensor,
    ...
    direct_light_env_light = dict_params.get("env_light")  # direct_env_light train.L72
    ...
    shs = None
    colors_precomp = None
    if override_color is None: # 最好有一个预设值好的颜色
    ...
    pbr = rendered_pbr
    rendered_pbr = pbr * rendered_opacity + (1 - rendered_opacity) * bg_color[:, None, None]  # 1.4
...
def calculate_loss(viewpoint_camera, pc, results, opt, direct_light_env_light):
    ...
    if opt.lambda_base_color_smooth > 0:  # （12）
        image_mask = viewpoint_camera.image_mask.cuda()
    ...
    if opt.lambda_light_smooth > 0:  # （12）
        image_mask = viewpoint_camera.image_mask.cuda()
...
def rendering_equation(base_color, roughness, normals, viewdirs,
    ...
    incident_visibility = visibility_precompute   # 算是 V
    ...
    f_s = GGX_specular(normals, viewdirs, incident_dirs, roughness, fresnel=0.04) # (8)
...
def GGX_specular(normal, pts2c, pts2l, roughness, fresnel):  # Real Shading in Unreal Engine 4
    ...
    NoL = torch.sum(N[:, None, :] * L, dim=-1, keepdim=True).clamp_(1e-6, 1)  # [nrays, nlights, 1] DO check broadcast
    ...
    nom1 = NoV * (1 - k) + k  #  G1(NV)
    nom2 = NoL * (1 - k[:, None, :]) + k[:, None, :]  # G2(NL)
