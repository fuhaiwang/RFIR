...
from scene.RenderLoS_NLoS_model import Renderer_LoS_NLoS



def render_view(viewpoint_antenna, pc: GaussianModel, pipe, bg_color: torch.Tensor, renderer_los_nlos, is_training=False, dict_params=None,
    ...
    origin_siganl = renderer_los_nlos.tx_power 

    
    direct_signal = origin_siganl   # 1e3*100
    ...
    offset_weight = renderer_los_nlos.compute_weight(LOS_dist, incident_visibility)
    rendered_CSI_abs = (1 - offset_weight) * torch.abs(rendered_CSI) + offset_weight * origin_siganl

    results = {"render": rendered_CSI_abs,
...
def calculate_loss(viewpoint_antenna, pc, results, opt, direct_light_env_light):
    ...
    rendered_abs = results["render"].unsqueeze(-1).to("cuda")

    gt_raw = viewpoint_antenna.original_image.cuda()  
    center_idx = 0

    gt_CSI = (gt_raw[center_idx, 0] + 1j * gt_raw[center_idx, 1])  
    sampled_gt_CSI = gt_CSI.reshape(-1, 1).to("cuda")

    gt_abs = torch.sqrt(sampled_gt_CSI.real ** 2 + sampled_gt_CSI.imag ** 2 + 1e-12) 

    L_abs = torch.abs(rendered_abs - gt_abs)  # v23

    tb_dict["l_abs"] = L_abs.item()

    lambda_abs = opt.lambda_abs
    lambda_Rs = opt.lambda_Rs
    loss = lambda_abs * L_abs + lambda_Rs * gt_abs
...
def rendering_equation_RF(roughness, normals, viewdirs,  # 一
    ...
    # incident_dirs  可以用过Tx的位置和 到每个高斯小球的位置来确定。


    deg = int(np.sqrt(incidents.shape[1]) - 1)
    global_incident_lights = direct_light_env_light.direct_light(incident_dirs)  # 全局入射光  一！
...
def render_RF_RSSI_prediction(viewpoint_antenna: Antenna, pc: GaussianModel, pipe, bg_color: torch.Tensor,
                 renderer_los_nlos: Renderer_LoS_NLoS, scaling_modifier=1.0, override_color=None, opt: OptimizationParams = False,
    ...
    results = render_view(viewpoint_antenna, pc, pipe, bg_color, renderer_los_nlos, is_training, dict_params,
