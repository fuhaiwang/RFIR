...
def render_view(viewpoint_antenna: Antenna, pc: GaussianModel, pipe, bg_color: torch.Tensor,
    ...
    intrinsic = viewpoint_antenna.intrinsics
    ...
    # 1.base_color  2.roughness  3.normal  4.incidents  5.viewdirs
    # base_color = pc.get_base_color
    roughness = pc.get_roughness
    normal = pc.get_normal
    incidents = pc.get_incidents
    viewdirs = F.normalize(viewpoint_antenna.camera_center - means3D, dim=-1)  # v1.0-4

    dir_pp = (pc.get_xyz - viewpoint_antenna.camera_center.repeat(pc.get_shs.shape[0], 1))  #
    dir_pp_normalized = F.normalize(dir_pp, dim=-1)

    direct_light_env_light = dict_params.get("env_light")  # v1.0-3

    if is_training:
        brdf_RF, extra_results = rendering_equation_RF(   # v1
            # base_color,
            roughness, normal.detach(), viewdirs, incidents,
        ...
        for i in range(0, means3D.shape[0], chunk_size):
            _brdf_RF, _extra_results = rendering_equation_RF(
    ...
    depths = (xyz_homo @ viewpoint_antenna.world_view_transform)[:, 2:3]
    depths2 = depths.square()

    if is_training:
        features = torch.cat([depths, depths2, brdf_RF, normal,

                              roughness,
                              extra_results["diffuse_light"], 
                              extra_results["incident_visibility"].mean(-2)], dim=-1)
    else:
        features = torch.cat([depths, depths2, brdf_RF, normal,   # 1,1,3,3,1,...
    ...
    feature_dict = {}
    if is_training:  #, rendered_base_color
        rendered_depth, rendered_depth2, rendered_pbr, rendered_normal, \
        rendered_roughness, rendered_diffuse, rendered_visibility \
            = rendered_feature.split([1, 1, 2, 3, 1, 3, 1], dim=0)
    ...
    else:  # rendered_base_color,
        rendered_depth, rendered_depth2, rendered_pbr, rendered_normal, \
            rendered_roughness, rendered_diffuse, rendered_specular, rendered_light, rendered_local_light, rendered_global_light, rendered_visibility \
            = rendered_feature.split([1, 1, 2, 3, 1, 3, 3, 3, 3, 3, 1], dim=0)
        feature_dict.update({
    ...
    if not is_training:
        directions = viewpoint_antenna.get_world_directions()
...
def calculate_loss(viewpoint_camera, pc, results, opt, direct_light_env_light):
    ...
    if opt.lambda_roughness_smooth > 0:  # （12）
        image_mask = viewpoint_camera.image_mask.cuda()
...
def render_neilf_RF(viewpoint_antenna: Antenna, pc: GaussianModel, pipe, bg_color: torch.Tensor,
    ...
    """
    results = render_view(viewpoint_antenna, pc, pipe, bg_color,
                          scaling_modifier, override_color, is_training, dict_params)  # 1.1
...
def rendering_equation(
    ...
    incident_dirs, incident_areas = incident_dirs_precompute, incident_areas_precompute  # incident_dirs_precompute需要提前准备好？
    ...
    incident_lights = local_incident_lights
...
def GGX_specular(normal, pts2c, pts2l, roughness, fresnel):  # Real Shading in Unreal Engine 4
    L = F.normalize(pts2l, dim=-1)  # [nrays, nlights, 3]
    V = F.normalize(pts2c, dim=-1)  # [nrays, 3]
    H = F.normalize((L + V[:, None, :]) / 2.0, dim=-1)  # [nrays, nlights, 3]    # 半角向量
    N = F.normalize(normal, dim=-1)  # [nrays, 3]  # 表面法向量

    NoV = torch.sum(V * N, dim=-1, keepdim=True)  # [nrays, 1]  cos(theta)
    ...
    FMi = ((-5.55473) * VoH - 6.98316) * VoH  # 出射
...
def rendering_equation_RF(roughness, normals, viewdirs,  # 一
    ...
    deg = int(np.sqrt(incidents.shape[1]) - 1)
    global_incident_lights = direct_light_env_light.direct_light(incident_dirs)
    local_incident_lights = eval_sh(deg, incidents.transpose(1, 2).view(-1, 1, 3, (deg + 1) ** 2),
...
def Degli_Esposti_scattering(normal, pts2c, pts2l, roughness, BSDF_class):
    if BSDF_class == "Degli-Esposti":
        L = F.normalize(pts2l, dim=-1)
        N = F.normalize(normal, dim=-1)  # [nrays, 3]  # 表面法向量
        NoL = torch.sum(N[:, None, :] * L, dim=-1, keepdim=True).clamp_(1e-6, 1)
        ...
        R = L - 2 * NoL * N
        cos_Phi_R = pts2c * R
        cos_Phi_R = np.clip(cos_Phi_R, -1.0, 1.0)

        f_thetai_phii_thetas_phis_ = ((1 + cos_Phi_R) / 2) ** roughness    # (10)
        f_thetai_phii_thetas_phis_ *= NoV ** 0.5             # (14)

        k_alpha_R = 1 / (0.07937 * roughness + 0.1745)
        F_alpha_R = NoL ** 0.5 * k_alpha_R

        beta = NoL / F_alpha_R * f_thetai_phii_thetas_phis_
        alpha = 180
    else:
        beta = 1/np.pi
        alpha = 180

    return np.sqrt(beta), alpha

def reflection_coefficient(theta_i, epsilon_r, sigma, freq):
    theta_i = np.asarray(theta_i)

    epsilon0 = 8.854187817e-12
    omega = 2 * np.pi * freq
    epsilon = epsilon_r * epsilon0 - 1j * sigma / omega


    n2 = np.sqrt(epsilon / epsilon0)
    n1 = 1.0
    ...
    rs = (n1 * np.cos(theta_i) - n2 * cos_t) / (n1 * np.cos(theta_i) + n2 * cos_t)
    rp = (n2 * np.cos(theta_i) - n1 * cos_t) / (n2 * np.cos(theta_i) + n1 * cos_t)
...
def rendering_equation_Wireless(roughness, normals, viewdirs,
                       incidents, direct_light_env_light=None,
                       visibility_precompute=None, incident_dirs_precompute=None, incident_areas_precompute=None):
    ...
    bate = Degli_Esposti_scattering(normals, viewdirs, incident_dirs, roughness, BSDF_class="Degli-Esposti")

    incidents = np.deg2rad(45)
    epsilon_r = 4.0
    simga = 0.0   #
    f = 5.8e9   #
    Chi = 180

    Chi, reflectance = Chi, reflection_coefficient(incidents, epsilon_r=4.0, simga=0, f=5.8e9)

    f_s = Degli_Esposti_scattering(normals, viewdirs, incident_dirs, roughness, BSDF_class="Degli-Esposti")

    transport = incident_lights * incident_areas * n_d_i  # （num_pts, num_sample, 3)
    specular = ((f_s) * reflectance * np.exp(1j * Chi) * transport).mean(dim=-2)
    pbr = (f_s * transport).mean(dim=-2)  # (8)
    diffuse_light = transport.mean(dim=-2)
...
if __name__ == "__main__":
    d = np.array([0, 0, -1])
    n = np.array([0, 0, 1])
    s = np.array([0.1, 0, 0.995])
    alpha = 10
    bsdf_val = Degli_Esposti_scattering(d, n, s, alpha)
    print("BSDF:", bsdf_val)
