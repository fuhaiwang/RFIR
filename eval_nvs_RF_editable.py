...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background, pbr_kwargs=None):
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        ...
        Render_abs = torch.abs( results["render"].cpu()).to("cuda") / args.power_scale
    ...
    ids = np.array(indexs)
    save_RF(gt_RFS, gts_path, "GT_RCS_360.png", ids, use_dB=True)
    save_RF_polar(gt_RFS, gts_path, "GT_RCS_360_polor.png", ids, use_dB=True)

    save_RF(Render_RFS, render_path, "Render_RCS_360.png", ids, use_dB=True)
    save_RF_polar(Render_RFS, render_path, "Render_RCS_360_polor.png", ids, use_dB=True)


def save_RF(RCS_list, path, file_name, indexs, use_dB=False):
...
def render_sets(dataset: ModelParams, pipeline: PipelineParams, skip_train: bool, skip_test : bool):
    with torch.no_grad():
        ...
        if args.checkpoint:
            ...
            gaussians.load_ply(os.path.join(args.output_path,
                                            "point_cloud",
        ...
        if not skip_train:
             render_set(args.output_path, "train", iteration, scene.getTrainCameras(), gaussians, pipeline, background, pbr_kwargs)

        if not skip_test:
             render_set(args.output_path, "test", iteration, scene.getTestCameras(), gaussians, pipeline, background, pbr_kwargs)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--power_scale", type=int, default=1)
    parser.add_argument("-output_path", "--output_path", type=str, default=None)
    parser.add_argument("-new_s", "--new_s", type=str, default=None)
    args = get_combined_args(parser)


    if args.new_s is not None:
        args.source_path = args.new_s
