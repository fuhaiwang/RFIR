...
def training(dataset: ModelParams, opt: OptimizationParams, pipe: PipelineParams, is_pbr=False, is_wideband=False):
    ...
    los_nlos = True
    if los_nlos:
        renderer_los_nlos = Renderer_LoS_NLoS(tx_power_init=1e-2)
        renderer_los_nlos.train_setting(lr=1e-3)
    ...
    for iteration in progress_bar: 
        ...
        if los_nlos:
            renderer_los_nlos.optimizer.zero_grad()
        ...
        render_pkg = render_fn(viewpoint_cam, gaussians, pipe, background, renderer_los_nlos, opt=opt, is_training=True, dict_params=pbr_kwargs, iteration=iteration)
        ...
        if torch.isnan(loss) or torch.isinf(loss):
            print("[Warn] loss is NaN/Inf, skip backward")
            renderer_los_nlos.optimizer.zero_grad()
            continue
        ...
        if iteration % 100 == 0:
            print(f"Iteration {iteration} | tx_power: {renderer_los_nlos.tx_power.item():.6f} | grad: {renderer_los_nlos.tx_power.grad}")
        ...
        with torch.no_grad():
            if pipe.save_training_vis:
                if is_pbr:
                    save_training_vis(viewpoint_cam, gaussians, background, render_fn, renderer_los_nlos,
            ...
            if is_pbr:
                training_report_RF(tb_writer, iteration, tb_dict,
                                    scene, render_fn, renderer_los_nlos, pipe=pipe,
            ...
            if los_nlos:
                renderer_los_nlos.optimizer.step()
            ...
            if iteration % args.checkpoint_interval == 0 or iteration == args.iterations:
                ...
                if los_nlos:
                    renderer_los_nlos.save_weights(args.model_path, iteration)
    ...
    if dataset.eval:
        if is_pbr:
            eval_RF_render(scene, gaussians, render_fn, renderer_los_nlos, pipe, background, opt, pbr_kwargs)


def training_report_RF(tb_writer, iteration, tb_dict, scene: Scene, renderFunc, renderer_los_nlos, pipe,
        ...
        for config in validation_configs:
            if config['cameras'] and len(config['cameras']) > 0:
                ...
                for idx, viewpoint in enumerate(
                    ...
                    render_pkg = renderFunc(viewpoint, scene.gaussians, pipe, bg_color, renderer_los_nlos, 
...
def sig2mse_abs(x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
    return torch.mean(torch.abs(x - y) ** 2)


def save_training_vis(viewpoint_cam, gaussians, background, render_fn, renderer_los_nlos, pipe, opt, first_iter, iteration, pbr_kwargs):
    ...
    with torch.no_grad():
        if iteration % pipe.save_training_vis_iteration == 0 or iteration == first_iter + 1:
            render_pkg = render_fn(viewpoint_cam, gaussians, pipe, background, renderer_los_nlos,
...
def eval_RF_render(scene, gaussians, render_fn, renderer_los_nlos, pipe, background, opt, pbr_kwargs):
    ...
    with torch.no_grad():
        for idx in progress_bar:
            ...
            results = render_fn(viewpoint, gaussians, pipe, background, renderer_los_nlos, opt=opt, is_training=False, dict_params=pbr_kwargs)

            Render_RF = torch.abs(results["render"].cpu()).to("cuda")
            Render_RFS.append(Render_RF)

            gt_raw = viewpoint.original_image[0, :].cuda()
            gt_CSI = (gt_raw[0] + 1j * gt_raw[1]) * 1e0
            ...
            if gaussians.use_pbr:
                ...
                min_ro_val = roughness_image.min()
                max_ro_val = roughness_image.max()
                normalized_roughness_image = (roughness_image - min_ro_val) / (max_ro_val - min_ro_val)
        ...
        save_RF(Render_RFS, os.path.join(args.model_path, 'eval', "render", "Render_RCS_360.png"), use_dB=True)

        save_RF(gt_RFS, os.path.join(args.model_path, 'eval', "gt", "gt_RCS_360.png"), use_dB=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-t', '--type', choices=['render', 'normal', 'render_RF', 'render_RF_RSSI_prediction'], default='render_RF_RSSI_prediction')   # neilf 是relightable 3D Gaussian
    ...
    args = parser.parse_args(sys.argv[1:])               # 确保只解析用户输入的参数
    ...
    is_pbr = args.type in ['render_RF', 'render_RF_RSSI_prediction']
    is_wideband = args.frequency in ['wide']
    training(lp.extract(args), op.extract(args), pp.extract(args), is_pbr=is_pbr, is_wideband=is_wideband)
