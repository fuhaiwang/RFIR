...
import cv2
from scene.utils import load_img_rgb, load_mask_bool, load_depth, load_pfm, load_mask
...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder, debug=False):
    ...
    for idx, key in enumerate(cam_extrinsics):
        ...
        mask_3ch = np.expand_dims(mask, axis=-1)  # (H, W, 1)
        mask_3ch = np.repeat(mask_3ch, 3, axis=-1)  # (H, W, 3)
...
def fetchPly(path):
    ...
    vertices = plydata['vertex']  # 一个面片的顶点索引
...
def readCamerasFromTransforms(path, transformsfile, white_background, extension=".png", debug=False):
    ...
    with open(os.path.join(path, transformsfile)) as json_file:
        ...
        for idx, frame in enumerate(tqdm(frames, leave=False)):
            ...
            cam_infos.append(CameraInfo(uid=idx, R=R, T=T, FovY=fovy, FovX=fovx, image=image, image_mask=image_mask,  # image_mask在这里加载，会被计算mask_loss时 调用。
...
def load_mitsuba_ply(ply_path):
    ...
    if 's' in vertex.dtype.names and 't' in vertex.dtype.names:
        st = np.vstack((vertex['s'], vertex['t'])).T
    else:
        st = None

    return xyz, normals, st
...
def readNerfSyntheticInfo(path, white_background, eval, extension=".png", debug=False):
    ...
    if not os.path.exists(ply_path):
        ...
        classroom_ply_path = os.path.join(path, "classroom_points3d1.ply")
        if os.path.exists(classroom_ply_path):
            xyz, normals, st = load_mitsuba_ply(classroom_ply_path)
            num_pts = xyz.shape[0]
        else:
            num_pts = 100_000  # lego
            # num_pts = 100_000  # classroom
            print(f"Generating random point cloud ({num_pts})...")
            # We create random points inside the bounds of the synthetic Blender scenes
            room_size = np.array([8.0, 11.0, 3.0])
            xyz = (np.random.random((num_pts, 3)) - 0.5) * room_size
...
def readAntennaFromTransforms(path, transformsfile, white_background, extension=".png", debug=False):
    Ant_infos = []
    ...
    with open(os.path.join(path, transformsfile)) as json_file:
        ...
        frames = contents["frames"]
        for idx, frame in enumerate(tqdm(frames, leave=False)):
            CSI = np.array(frame["CSI"])
            view_index = frame["file_path"]
            # NeRF 'transform_matrix' is a camera-to-world transform
            c2w = np.array(frame["transform_matrix"])
            # change from OpenGL/Blender camera axes (Y up, Z back) to COLMAP (Y down, Z forward)
            c2w[:3, 1:3] *= -1  # raw
            ...
            Ant_infos.append(AntennaInfo(uid=idx, R=R, T=T, FovY=fovy, FovX=fovx, RF=CSI, image_mask=None,
                                        image_path=None, depth=None, normal=None, image_name=view_index,
                                        width=image_width, height=image_height, Tx_position=Tx_antenna_pos, Rx_position=Tx_antenna_pos))
...
def readOmnidirAntennaFromTransforms_v1(path, transformsfile, white_background, extension=".png", debug=False):
    Ant_infos = []
    ...
    with open(os.path.join(path, transformsfile)) as json_file:
        ...
        for idx, frame in enumerate(tqdm(frames, leave=False)):
            ...
            Tx_antenna_pos = contents["Tx_position"]
            Rx_antenna_pos = c2w[:3, 3]
            ...
            Ant_infos.append(AntennaInfo(uid=idx, R=R, T=T, FovY=fovy, FovX=fovx, RF=CSI, image_mask=None,
                                        image_path=None, depth=None, normal=None, image_name=view_index,
                                        width=image_width, height=image_height, Tx_position=Tx_antenna_pos, Rx_position=Rx_antenna_pos))

    return Ant_infos


def readOmnidirAntennaFromTransforms(path, transformsfile, white_background, extension=".png", debug=False):
    Ant_infos = []
    ...
    with open(os.path.join(path, transformsfile)) as json_file:
        ...
        if isinstance(raw_data, dict):
            contents_all = [raw_data]
        else:
            contents_all = raw_data

        for contents in contents_all:
            frames = contents["frames"]
            Tx_antenna_pos = contents["Tx_position"]
    ...
    return Ant_infos  # viewpoint_cam


def RFantennaSimuInfo(path, white_background, eval, extension=".png", debug=False):
    print("Reading Training Transforms")
    train_cam_infos = readAntennaFromTransforms(path, "RF_antenna_info_train.json", white_background, extension, debug=debug)
    if eval:
        print("Reading Test Transforms")
        test_cam_infos = readAntennaFromTransforms(path, "RF_antenna_info_test.json", white_background, extension,
                                                   debug=debug)
    else:
        test_cam_infos = []
    ...
    if not os.path.exists(ply_path):
        ...
        # We create random points inside the bounds of the synthetic Blender scenes
        xyz = np.random.random((num_pts, 3)) * 5  # 2.6 - 1.3   car=5
        shs = np.random.random((num_pts, 3)) / 255.0
        normals = np.random.randn(*xyz.shape)
        normals /= np.linalg.norm(normals, axis=-1, keepdims=True)
...
def RFOmnidirantennaSimuInfo(path, white_background, eval, extension=".png", debug=False):   # lego的文件配置读取
    print("Reading Training Transforms")
    train_cam_infos = readOmnidirAntennaFromTransforms(path, "RF_omnidir_antenna_info_train.json", white_background, extension, debug=debug)
    if eval:
        print("Reading Test Transforms")
        test_cam_infos = readOmnidirAntennaFromTransforms(path, "RF_omnidir_antenna_info_test.json", white_background, extension,
                                                   debug=debug)
    else:
        test_cam_infos = []
...
sceneLoadTypeCallbacks = {
    ...
    "RF_antenna_simulation": RFantennaSimuInfo,
    "RF_omnidir_antenna_simulation": RFOmnidirantennaSimuInfo
}
