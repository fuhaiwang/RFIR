...
from utils.camera_utils import cameraList_from_camInfos, antennaList_from_antInfos, camera_to_JSON
    ...
    def __init__(self, args: ModelParams, gaussians: GaussianModel, load_iteration=None, shuffle=True,
        ...
        if os.path.exists(os.path.join(args.source_path, "sparse")):
            ...
            scene_info = sceneLoadTypeCallbacks["NeILF"](args.source_path, args.white_background, args.eval,
                                                         debug=args.debug_cuda)
        elif os.path.exists(os.path.join(args.source_path, "RF_antenna_info_train.json")):
            print("Found RF_antenna_info.json file, assuming Blender-like data set!")
            scene_info = sceneLoadTypeCallbacks["RF_antenna_simulation"](args.source_path, args.white_background, args.eval,
                                                         debug=args.debug_cuda)
        elif os.path.exists(os.path.join(args.source_path, "RF_omnidir_antenna_info_train.json")):
            print("Found RF_antenna_info.json file, assuming Blender-like data set!")
            scene_info = sceneLoadTypeCallbacks["RF_omnidir_antenna_simulation"](args.source_path, args.white_background, args.eval,
                                                         debug=args.debug_cuda)
        else:
        ...
        for resolution_scale in resolution_scales:
            if os.path.exists(os.path.join(args.source_path, "RF_antenna_info_train.json")):
                print("Loading Training Antennas")
                self.train_cameras[resolution_scale] = antennaList_from_antInfos(scene_info.train_cameras,
                                                                                resolution_scale,
                                                                                args)
                print("Loading Test Antennas")
                self.test_cameras[resolution_scale] = antennaList_from_antInfos(scene_info.test_cameras,
                                                                               resolution_scale,
                                                                               args)
            elif os.path.exists(os.path.join(args.source_path, "RF_omnidir_antenna_info_train.json")):
                print("Loading Training Antennas")
