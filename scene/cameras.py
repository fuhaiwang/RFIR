...
class Antenna(nn.Module):
    def __init__(self, colmap_id, R, T, FoVx, FoVy, RF, RF_name, image_width, image_height, uid, Tx_pos, Rx_pos,
        ...
        self.FoVx = FoVx
        self.FoVy = FoVy
        self.image_width = image_width  # 360
        self.image_height = image_height  # 90
        self.image_name = RF_name

        if RF is not None:
            self.original_image = torch.tensor(RF, device=data_device)
        ...
        self.Tx_pos = torch.tensor(Tx_pos).cuda()
        self.Rx_pos = torch.tensor(Rx_pos).cuda()
        self.Tx_power = torch.tensor(1e7).cuda()  
