...
class GaussianModel:
    ...
    def setup_functions(self):
        ...
        if self.use_pbr:
            self.roughness_activation = lambda x: F.softplus(x) + 1.0  
            self.inverse_roughness_activation = lambda y: torch.log(torch.exp(y - 1.0) - 1.0 + 1e-8)   
            self.reflection_coe_activation = lambda x: x # 
            self.reflection_phase_activation = lambda x: torch.sigmoid(x) * (2 * math.pi)  # [0,2pi]


    def __init__(self, sh_degree: int, render_type='render'):
        self.render_type = render_type
        self.use_pbr = render_type in ['render_RF', 'render_RF_RSSI_prediction']
        ...
        self._visibility_tracing = None   # 预计算
        ...
        if self.use_pbr:
            ...
            self._reflection_coe = torch.empty(0)
            self._reflection_phase = torch.empty(0)
            ...
            self._incident_distances = torch.empty(0)  #
    ...
    def restore(self, model_args, training_args,
        ...
        if len(model_args) > 15 and self.use_pbr:
            (
             self._roughness,
             self._reflection_coe,    # my
             self._reflection_phase,  # my
             self._incidents_dc,
             self._incidents_rest,
             self._visibility_dc,
    ...
    @property
    def get_reflection_coe(self):
        return self.reflection_coe_activation(self._reflection_coe)

    @property
    def get_reflection_phase(self):
        return self.reflection_phase_activation(self._reflection_phase)
    ...
    @torch.no_grad()
    def update_visibility_RF_single(self, cam):
        raytracer = RayTracer(self.get_xyz, self.get_scaling, self.get_rotation)
        gaussians_xyz = self.get_xyz
        gaussians_inverse_covariance = self.get_inverse_covariance()
        gaussians_opacity = self.get_opacity[:, 0]
        ...
        Tx_pos = torch.as_tensor(cam.Tx_pos, device=gaussians_xyz.device, dtype=gaussians_xyz.dtype).view(1, 3)
        incident_visibility_results = []
        incident_dirs_results = []
        incident_distances_results = []

        chunk_size = gaussians_xyz.shape[0] // ((64 - 1) // 24 + 1)
        for offset in range(0, gaussians_xyz.shape[0], chunk_size):
            xyz_chunk = gaussians_xyz[offset:offset + chunk_size]
            incident_dirs = F.normalize(Tx_pos - xyz_chunk, dim=-1)  # [C, 3]
            incident_distances = torch.norm(Tx_pos - xyz_chunk, dim=-1, keepdim=True)
            trace_results = raytracer.trace_visibility(  # C++ 接口
                xyz_chunk,
                incident_dirs,
                gaussians_xyz,
                gaussians_inverse_covariance,
            ...
            incident_visibility_results.append(incident_visibility)
            incident_dirs_results.append(incident_dirs)
            incident_distances_results.append(incident_distances)

        incident_visibility_result = torch.cat(incident_visibility_results, dim=0)
        incident_dirs_result = torch.cat(incident_dirs_results, dim=0)
        incident_distances_result = torch.cat(incident_distances_results, dim=0)

        self._visibility_tracing = incident_visibility_result
        self._incident_dirs = incident_dirs_result
        self._incident_distances = incident_distances_result
    ...
    def create_from_ckpt(self, checkpoint_path, restore_optimizer=False):
        ...
        if self.use_pbr:
            if len(model_args) > 15:
                ...
                roughness = torch.ones_like(self._xyz[..., :1])
                self._roughness = nn.Parameter(roughness.requires_grad_(True))

                reflection_coe = torch.ones_like(self._xyz[..., :1])
                self._reflection_coe = nn.Parameter(reflection_coe.requires_grad_(True))

                reflection_phase = torch.ones_like(self._xyz[..., :1])
                self._reflection_phase = nn.Parameter(reflection_phase.requires_grad_(True))
    ...
    def create_from_RF_ckpt(self, checkpoint_path, restore_optimizer=False):
        (model_args, first_iter) = torch.load(checkpoint_path)
    ...
    def create_from_pcd(self, pcd: BasicPointCloud, spatial_lr_scale: float):
        ...
        self._normal = nn.Parameter(fused_normal.requires_grad_(True))    # self._normal
        self._rotation = nn.Parameter(rots.requires_grad_(True))   # 椭球信息 旋转初始化ok
        self._scaling = nn.Parameter(scales.requires_grad_(True))     # 椭球信息 缩放初始化ok
        self._opacity = nn.Parameter(opacities.requires_grad_(True))  # 椭球透明度
        self._shs_dc = nn.Parameter(shs[:, :, 0:1].transpose(1, 2).contiguous().requires_grad_(True))  # 直流
        self._shs_rest = nn.Parameter(shs[:, :, 1:].transpose(1, 2).contiguous().requires_grad_(True))  # 高阶
        ...
        if self.use_pbr:
            ...
            reflection_coe = torch.zeros((fused_point_cloud.shape[0], 1), dtype=torch.float, device="cuda")
            self._reflection_coe = nn.Parameter(reflection_coe.requires_grad_(True))

            reflection_phase = torch.zeros((fused_point_cloud.shape[0], 1), dtype=torch.float, device="cuda")
            self._reflection_phase = nn.Parameter(reflection_phase.requires_grad_(True))
    ...
    def training_setup(self, training_args: OptimizationParams):
        self.percent_dense = training_args.percent_dense    # 0.001
        ...
        if self.use_pbr:
            ...
            l.extend([
                ...
                {'params': [self._reflection_coe], 'lr': training_args.reflection_coe_lr, "name": "reflection_coe"},
                {'params': [self._reflection_phase], 'lr': training_args.reflection_phase_lr, "name": "reflection_phase"},
            ])
    ...
    def construct_list_of_attributes(self):
        ...
        if self.use_pbr:
            l.append('roughness')
            l.append('reflection_coe')
            l.append('reflection_phase')
    ...
    def save_ply(self, path):
        ...
        if self.use_pbr:
            attributes_list.extend([
                ...
                self._reflection_coe.detach().cpu().numpy(),
                self._reflection_phase.detach().cpu().numpy(),
    ...
    def load_ply(self, path):
        ...
        self._normal = nn.Parameter(torch.tensor(normal, dtype=torch.float, device="cuda").requires_grad_(True))  # self._normal
        ...
        if self.use_pbr:
            ...
            reflection_coe = np.asarray(plydata.elements[0]["roughness"])[..., np.newaxis]
            reflection_phase = np.asarray(plydata.elements[0]["roughness"])[..., np.newaxis]

            self._roughness = nn.Parameter(
                torch.tensor(roughness, dtype=torch.float, device="cuda").requires_grad_(True))
            self._reflection_coe = nn.Parameter(
                torch.tensor(reflection_coe, dtype=torch.float, device="cuda").requires_grad_(True))
            self._reflection_phase = nn.Parameter(
                torch.tensor(reflection_phase, dtype=torch.float, device="cuda").requires_grad_(True))
            ...
            self._incidents_rest = nn.Parameter(
                torch.tensor(incidents_extra, dtype=torch.float, device="cuda").transpose(
                    1, 2).contiguous().requires_grad_(True))    # 这里定义了网络参数化的 入射角度 _incidents_rest
    ...
    def prune_points(self, mask):
        ...
        if self.use_pbr:
            ...
            self._reflection_coe = optimizable_tensors["reflection_coe"]
            self._reflection_phase = optimizable_tensors["reflection_phase"]
